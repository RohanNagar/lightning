...
public class FacebookResourceTest {
  ...
  public void setup() {
    ...
    properties.put("facebook-access-token", "fbAccessToken");
  ...
  //    RetrofitError error = mock(RetrofitError.class);
  //    when(error.getResponse()).thenReturn(new retrofit.client.Response("url", 401,
  //        "Unauthorized", Lists.newArrayList(), null));
  //    when(thunderClient.getUser(anyString(), anyString())).thenThrow(error);
  //
  //    Response response = resource.getVideos(key, "Test", "password");
  //
  ...
  //        PublishType.fromString("Fake"), "Test Message", inputStream, contentDisposition,
  //        "Test Title");
  //
  //    assertEquals(response.getStatusInfo(), Response.Status.BAD_REQUEST);
  //  }
  ...
  //    when(thunderClient.updateUser(any(User.class), anyString(), anyString())).thenThrow(error);
  //
  //    Response response = resource.getExtendedToken(key, "Test", "password");
  //
  //    assertEquals(response.getStatusInfo(), Response.Status.NOT_FOUND);
  ...
  //    when(facebookService.getFacebookExtendedToken()).thenReturn("Test");
  //    when(thunderClient.updateUser(any(User.class), anyString(), anyString()))
  //        .thenReturn(user);
  //
  //    Response response = resource.getExtendedToken(key, "Test", "password");
